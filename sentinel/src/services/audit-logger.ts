 ...
 */
export async function logAction(guild: Guild, type: string, description: string): Promise<void> {
...
export async function logSummary(
  ...
  if (channel) {
    try {
      ...
      console.error(`[AUDIT] Failed to post summary to #${AUDIT_LOG_CHANNEL}:`, err);
    }
  }
}
