...
const command: Command = {
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    const embed = reportType === 'weekly' ? buildWeeklyEmbed() : buildChecklistEmbed();

    const label = reportType === 'weekly' ? 'Weekly report' : 'Daily checklist';

    // Post to #ops
    const message = await opsChannel.send({ embeds: [embed] });

    // Audit log
    await logAction(guild, 'REPORT', `\u{1f4cb} ${label} posted in #${DESTINATION_CHANNEL}`);
