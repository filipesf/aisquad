...
const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        .setName('full')
        .setDescription('Run the full server setup from the architecture spec (idempotent)')
    ...
    .addSubcommand((sub) =>
      sub
        .setName('update')
        .setDescription('Reconcile server to match architecture — creates missing, updates drifted')
    )
    .addSubcommand((sub) =>
      sub
        .setName('verify')
        .setDescription('Check current server state against the architecture spec (read-only)')
    ),
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (subcommand === 'full') {
      ...
      const embed = new EmbedBuilder()
        .setColor(result.errors.length > 0 ? EMBED_COLORS.WARNING : EMBED_COLORS.SUCCESS)
    ...
    if (subcommand === 'update') {
      ...
      const embed = new EmbedBuilder()
        ...
        .setTitle(totalChanges > 0 ? 'Setup Update Complete' : 'No Changes Needed')
        .addFields(
          {
            name: 'Roles',
