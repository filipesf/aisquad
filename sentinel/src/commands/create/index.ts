...
const command: Command = {
  data: new SlashCommandBuilder()
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((opt) =>
          opt.setName('color').setDescription('Hex color (e.g. #3498db)').setRequired(true)
        )
    ...
    .addSubcommand((sub) =>
      sub
        ...
        .addStringOption((opt) => opt.setName('topic').setDescription('Channel topic'))
    )
    .addSubcommand((sub) =>
      sub
        .setName('category')
  ...
  async execute(interaction: ChatInputCommandInteraction) {
    ...
    if (sub === 'role') {
      ...
      await logAction(guild, 'ROLE', `Created @${name} by ${interaction.user.tag}`);
    ...
    if (sub === 'category') {
      ...
      await logAction(guild, 'CATEGORY', `Created ${name} by ${interaction.user.tag}`);
