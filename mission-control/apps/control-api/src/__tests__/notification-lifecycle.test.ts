...
describe('notification lifecycle', () => {
  ...
  it('enqueues a notification', async () => {
    const notif = await notificationDomain.enqueue('agent-1', 'comment', 'comment-1', {
      body: 'hello'
    });
  ...
  it('deduplicates notifications within window', async () => {
    const first = await notificationDomain.enqueue('agent-1', 'comment', 'comment-1', {
      body: 'hello'
    });
    const second = await notificationDomain.enqueue('agent-1', 'comment', 'comment-1', {
      body: 'hello'
    });
  ...
  it('allows notifications for different sources', async () => {
    const n1 = await notificationDomain.enqueue('agent-1', 'comment', 'comment-1', {
      body: 'first'
    });
    const n2 = await notificationDomain.enqueue('agent-1', 'comment', 'comment-2', {
      body: 'second'
    });
  ...
  it('allows notifications for different targets', async () => {
    ...
    const n2 = await notificationDomain.enqueue('agent-2', 'comment', 'comment-1', {
      body: 'hello'
    });
