...
import { ApiError, getTelemetrySummary } from '@/lib/api';
import type { TelemetryGroupBy, TelemetryWindow } from '@/types/domain';
import { TELEMETRY_GROUP_BY_OPTIONS, TELEMETRY_WINDOWS } from '@/types/domain';
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex flex-wrap items-center gap-4">
        ...
        <div className="ml-auto flex flex-wrap items-center gap-3">
          <span className="text-muted-foreground text-xs" id="window-label">
            Period
          </span>
          <ToggleGroup
            type="single"
          ...
          >
            {TELEMETRY_WINDOWS.map((w) => (
              <ToggleGroupItem key={w} value={w} className="h-8 px-3 text-xs">
                {w}
              </ToggleGroupItem>
            ))}
          </ToggleGroup>
          <span className="text-muted-foreground text-xs" id="groupby-label">
            Group by
          </span>
          <ToggleGroup
            type="single"
          ...
          >
            {TELEMETRY_GROUP_BY_OPTIONS.map((g) => (
              <ToggleGroupItem key={g} value={g} className="h-8 px-3 text-xs capitalize">
                {g.replace(/_/g, ' ')}
              </ToggleGroupItem>
            ))}
          </ToggleGroup>
      ...
      {data && (
        <>
          ...
          {data.groups.length > 0 ? (
            ...
            <p className="text-muted-foreground text-sm">No data for this time period.</p>
          )}
      ...
      {loading && !data && (
        ...
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6" aria-hidden="true">
          {(['events', 'tokens', 'cost', 'avg-lat', 'min-lat', 'max-lat'] as const).map((k) => (
            <div key={k} className="skeleton-shimmer rounded-md border bg-muted/40 px-4 py-3">
