...
describe('notifications integration', () => {
  ...
  it('notifies all subscribers except the author', async () => {
    ...
    await post<CommentResp>(
      `/tasks/${task.id}/comments?author_id=${sub1.id}`,
      { body: 'Subscribing myself' },
    );
    await post<CommentResp>(
      `/tasks/${task.id}/comments?author_id=${sub2.id}`,
      { body: 'Me too' },
    );
  ...
  it('notification dedup prevents duplicates within window', async () => {
    ...
    // Subscribe watcher
    await post<CommentResp>(
      `/tasks/${task.id}/comments?author_id=${subscriber.id}`,
      { body: 'Watching this' },
    );
    ...
    const { data: c1 } = await post<CommentResp>(
      `/tasks/${task.id}/comments?author_id=${author.id}`,
      { body: 'First rapid comment' },
    );
    const { data: c2 } = await post<CommentResp>(
      `/tasks/${task.id}/comments?author_id=${author.id}`,
  ...
  it('GET /agents/:id/notifications returns notifications', async () => {
    ...
    await post<CommentResp>(
      `/tasks/${task.id}/comments?author_id=${target.id}`,
      { body: 'Subscribing' },
    );
    await cleanRedis();
    await post<CommentResp>(
  ...
  it('POST /notifications/:id/ack marks notification as delivered', async () => {
    ...
    const { data: notifs } = await get<NotificationResp[]>(
      `/agents/${target.id}/notifications`,
    );
    expect(notifs.length).toBeGreaterThanOrEqual(1);
